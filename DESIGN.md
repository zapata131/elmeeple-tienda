...
*   **Regional Domestic Toggle & Tactile Switches (US-27 & US-32, Issues #18 & #35):** By default, game comparison tables and global toolbar settings activate `Solo tiendas de mi país` (`onlyDomestic = true`, `domesticOnly = true`) to protect players from foreign customs taxes and expensive shipping. All boolean toggle controls use modern accessible tactile switches (`role="switch"`, `aria-checked`) with event propagation stopped (`onClick={(e) => e.stopPropagation()}`) against parent container clicks.
