...
*   **Navbar Navigation & Brand Identity (US-30, US-39, Issues #33, #54):** Responsive navigation header (`Toolbar.tsx`) rendering clean SVG vector icons, streamlined role switcher pills (`Comprador`, `Tienda`, `Admin`), persistent brand branding (`MeeplePrecios`), and role-specific navigation row. When active role is `admin`, it displays a direct link to `/admin/dashboard` formatted in strict sentence case (`Panel de admin`) alongside administration links (`Tipos de cambio FX`, `Cola metadatos BGG`), bypassing partner and player links. All navigation items follow Google Developer Documentation Style Guide sentence case rules.
