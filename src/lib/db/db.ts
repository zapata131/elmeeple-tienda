...
export class DatabaseRepository {
  ...
  updateStoreSettings(
    storeId: string,
    updates: Partial<Store> & { flat_rate_shipping?: number; free_shipping_threshold?: number | null }
  ): Store | undefined {
    const store = this.stores.find(s => s.id === storeId);
    if (!store) return undefined;

    if (updates.name !== undefined) store.name = updates.name;
    if (updates.logo_url !== undefined) store.logo_url = updates.logo_url;
    if (updates.feed_url !== undefined) store.feed_url = updates.feed_url;
    if (updates.feed_status !== undefined) store.feed_status = updates.feed_status;

    if (updates.flat_rate_shipping !== undefined) {
      this.setShippingRate(storeId, updates.flat_rate_shipping, updates.free_shipping_threshold);
    }
  ...
  getBggTop10(): BggGame[] {
    const sorted = [...this.bggGames]
      .filter(g => typeof g.bgg_rank === 'number')
      .sort((a, b) => (a.bgg_rank ?? 999) - (b.bgg_rank ?? 999));
    return sorted.slice(0, 10);
  }

  getMostSearchedGames(): BggGame[] {
    const sorted = [...this.bggGames]
      .filter(g => typeof g.search_count === 'number')
  ...
  getOffersForGame(bggId: number): CalculatedOffer[] {
    const parentGame = this.getBggGameById(bggId);
    const isBaseGame = parentGame?.item_type === 'boardgame';
    const isEuropaGame = bggId === 14996 || parentGame?.name.toLowerCase().includes('europa');

    const offers = this.offers.filter(o => {
      if (o.bgg_id !== bggId || o.stock <= 0) return false;
      const urlLower = o.store_product_url.toLowerCase();

      // Systemic Entity Type Guard: If parent game is base game, exclude offers with expansion/promo keywords
      if (isBaseGame) {
        const expansionKeywords = ['expansion', 'expansio', 'ampliacion', 'ampliacio', 'posadas', 'circos', 'torres', 'navegantes'];
        if (expansionKeywords.some(kw => urlLower.includes(kw))) {
          return false;
        }
      }

      // Systemic Sub-edition Specificity Guard: Europa sub-edition games MUST contain 'europa' in URL
      if (isEuropaGame && !urlLower.includes('europa')) {
        return false;
      }
