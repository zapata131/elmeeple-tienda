...
export default function StoreOffersComparisonTable({
  ...
  const availableBasePrices = filteredOffers
    .filter((o) => o.stock > 0)
    .map((o) => o.price);
  const minBasePrice = availableBasePrices.length > 0 ? Math.min(...availableBasePrices) : null;
          ...
          <tbody className="divide-y divide-gray-200 text-sm">
            {filteredOffers.length === 0 ? (
              ...
              filteredOffers.map((offer) => {
                ...
                const isBestBasePriceOffer = minBasePrice !== null && offer.stock > 0 && offer.price === minBasePrice;
                  ...
                  <tr key={offer.id} data-testid={`store-offer-row-${offer.id}`} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col items-start gap-1">
                        ...
                        {isBestBasePriceOffer && (
                          <span
                            data-testid="best-price-badge-base"
                          ...
                          >
                            ...
                            <span>★ Mejor precio sin envío</span>
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      {offer.stock > 0 ? (
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center justify-center gap-1.5 px-4 py-2 text-xs font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 active:bg-gray-300 border border-gray-300 rounded-xl shadow-2xs hover:shadow-xs transition-all duration-200 group"
                        >
                          <span>Ver en tienda (Agotado)</span>
                          <svg className="w-3.5 h-3.5 text-gray-500 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-200 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                          </svg>
                        </a>
                      )}
