...
export async function verifyStoreOfferUrl(
  ...
  try {
    ...
    return {
      ...
      productTitleFound: null,
      isCorrectProductMatch: false,
      failureReason: err.message || 'Network fetch error',
    };
  }
}
  ...
  for (let i = 0; i < storeGames.length; i += BATCH_SIZE) {
    ...
    const chunkResults = await Promise.all(
      chunk.map(async (offer) => {
        const storeName = (offer.stores as any)?.name || 'Desconocida';
        const targetGameName = (offer.bgg_games_cache as any)?.name || `BGG ${offer.bgg_id}`;

        const audit = await verifyStoreOfferUrl(offer.store_product_url, targetGameName);

        if (!audit.isValidLink || (audit.failureReason && audit.failureReason.includes('expansion/accessory'))) {
          await supabase.from('store_games').delete().eq('id', offer.id);
        }
