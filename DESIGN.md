...
*   **Regional Domestic Toggle & Tactile Switches (US-27, US-32, US-36, Issues #18, #35, #44):** By default, game comparison tables (`StoreOffersComparisonTable.tsx`), main catalog search hero (`page.tsx`), and catalog filter panels (`CatalogView.tsx`) render contextual tactile switches (`RegionalStoreToggle.tsx`) activating `Solo tiendas en mi país` (`onlyDomestic = true`, `domesticOnly = true`) to protect players from foreign customs taxes and expensive shipping. Redundant domestic store toggles have been removed from the global navigation header (`Toolbar.tsx`). All boolean toggle controls use modern accessible tactile switches (`role="switch"`, `aria-checked`) with event propagation stopped (`onClick={(e) => e.stopPropagation()}`) against parent container clicks.
...
*   **Navbar Navigation & Brand Identity (US-30, US-39, US-44, Issues #33, #54, #62):** Responsive navigation header (`Toolbar.tsx`) rendering clean SVG vector icons, streamlined role switcher pills (`Comprador`, `Tienda`, `Admin`), persistent brand branding (`MeeplePrecios`), and a static sentence-case market lock badge (`México · $ MXN`) focusing the platform strictly on Mexico (`MX`) and Mexican Pesos (`MXN`) for commercial launch. When active role is `admin`, it displays a direct link to `/admin/dashboard` formatted in strict sentence case (`Panel de admin`) alongside administration links (`Tipos de cambio FX`, `Cola metadatos BGG`), bypassing partner and player links. All navigation items follow Google Developer Documentation Style Guide sentence case rules.
*   **Automated Sentence Case Governance & UI Harmonization (US-40, Issue #55):** All user-facing headings (`h1`, `h2`, `h3`), interactive buttons, table headers, and badges strictly follow sentence case according to the Google Developer Documentation Style Guide (e.g., `Comparativa de ofertas por tienda`, `Valoraciones y vibe tags`, `Selecciona tu lista de compra (wishlist)`). Compliance is continuously verified by `src/__tests__/sentence_case_style.test.tsx` and integrated into `npm run verify`.
*   **Layout SEO Targeting & Responsive Architecture (US-31, Issue #34):** Root layout (`src/app/layout.tsx`) and game detail pages (`src/app/game/[id]/page.tsx`) generate comprehensive dynamic SEO metadata, canonical URLs, and OpenGraph/Twitter card tags targeting Latin American and Iberian tabletop communities across desktop and mobile viewports.
*   **Edition Language Badges & Vector Pill Tags (US-33, Issue #36):** Comparison tables (`StoreOffersComparisonTable.tsx`) replace raw language text/flags with high-contrast, brand-styled typographic badge components (`renderEditionBadge`) displaying standardized edition markers (`ES · Español`, `PT · Português`, `EN · English`, `DE · Deutsch`, `MULTI · Multi-*   **Vector SVG Icons & Complete Emoji Ban (US-34, US-37, Issues #37, #47):** All user-facing interfaces, catalog search bars (`SearchBar.tsx`), cart optimizers (`CartOptimizerPanel.tsx`), review panels (`StoreReviewPanel.tsx`), and all self-serve administration (`/admin/*`) and merchant onboarding/diagnostics portals (`/merchant/*`) completely eradicate raw unicode emojis (`🎲`, `🔥`, `🌍`, `💸`, `📦`, `🇪🇸`, `🇵🇹`, `🇬🇧`, `🇩🇪`, `🌐`, `🏪`, `🛡️`, `ℹ️`, `✨`, `⚠️`, `🔔`, `⏳`, `⚡`, `🔒`, `🏷️`, `⭐`). They are replaced by crisp vector SVG icons and styled typography badges (`★`, `☆`) colored using official brand tokens (`#8367C7` Malva, `#73D8D4` Turquesa, `#FF9E8A` Coral).
*   **Pure BGG Wishlist Synchronization & Discount Alert Removal (US-35, Issue #42):** To decouple wishlist synchronization from manual discount price targeting, discount alert comparison grids and target price editing forms have been removed across the platform (`PriceAlertForm` returns null). The wishlist portal (`/dashboard/alerts`) relies on pure BGG collection synchronization fetching both `wishlist=1` and `wanttobuy=1` XML endpoints, matching games via Section 5.1 guidelines (`bgg_games_cache` and `games` lookup by `bgg_id`, then case-insensitive `name.ilike` or `alternate_names` match), guarding returns per Section 5.4, storing `target_price: null` in `price_alerts`, and displaying direct Best Price CTAs (`Ver Ofertas`).
*   **Code Quality & ESLint Governance (Issue #48):** To maintain the highest standard of engineering quality, type safety, and zero-regression reliability, the platform strictly enforces zero ESLint errors and warnings across all files (`eslint.config.mjs`). Unused parameters and callback arguments must follow standard TypeScript underscore notation (`argsIgnorePattern: "^_"`), and `@next/next/no-img-element` is configured to `off` to accommodate dynamic multi-domain external merchant product images from XML feeds without breaking production optimization pipelines.
*   **Automated Affiliate Tracking & Merchant Analytics Dashboard (US-10, Issue #46):** All outbound buy links across the platform (`CartOptimizerPanel.tsx`, catalog offers table) route through `/api/redirect`. The redirect endpoint (`GET /api/redirect`) automatically injects standard affiliate tracking parameters (`ref=meepleprecios&utm_source=meepleprecios&utm_medium=affiliate`) into any destination URL and asynchronously records referral clicks (`clicks` relation). Store owners access real-time referral metrics in `/merchant/dashboard`, viewing interactive daily/weekly click charts (`MerchantAnalyticsCharts.tsx`), top referred board games ranking tables, and UTM reconciliation guides formatted without raw unicode emojis.
*   **Historical Best-Price Deal Badge & Market Bargain Indicator (US-38, Issue #52):** Comparison tables (`StoreOffersComparisonTable.tsx`) and catalog grid cards (`CatalogView.tsx`) dynamically compute whether an offer matches the current lowest market price (`★ Mejor Precio Actual`) or is equal to/within 3% of the game's all-time historical minimum price (`★ Récord Mínimo Histórico`, `★ Mínimo Histórico`). All deal badges render high-contrast brand tokens (`#73D8D4` Turquesa, `#FF9E8A` Coral) with vector SVG icons and sentence-case typography, strictly preserving zero raw unicode emojis.
...
*   **Wishlist Synchronization & Restock Monitor Portal (`US-20`, `US-24`, `US-35`):** Registered players can subscribe to instant out-of-stock restock notifications (`RestockAlertButton.tsx` & `/api/user/restock-alert`). Features an interactive management dashboard (`/dashboard/alerts`) showing live inventory replenishment status and a 1-click BGG Wishlist Importer (`POST /api/user/sync-bgg`) that queries both `wishlist=1` and `wanttobuy=1` endpoints, storing synced games with `target_price: null` and rendering Best Price CTAs (`Ver Ofertas`) without discount comparison grids.
...
*   **Database (Supabase / PostgreSQL):**
    ...
    *   `price_alerts`: User wishlist and price notification records. Columns: `id`, `bgg_id`, `user_email`, `target_price` (numeric, nullable; stored as `null` for pure BGG wishlist synchronized games per `US-35`), `created_at`.
